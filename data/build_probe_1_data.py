...
def main(_):
  ...
  if (not tf.io.gfile.exists("gs://e2e_central/data/probes/movie_id_info.json") or 
  ...
  if (not tf.io.gfile.exists("gs://e2e_central/data/probes/probe_1.tsv") or
    ...
    def get_related_movies(movie, k=5):
      id = movie_ids["movie_to_id"][movie]
      row = mi_matrix[id]
    ...
    for movie in filtered_movies:
      ...
      for related, rand in zip(related_list, random_list):
        probes.append(f"[User] Can you recommend me a movie like @ {movie} @\tSure, have you seen @ {related} @?")
        probes.append(f"[User] Can you recommend me a movie like @ {movie} @\tSure, have you seen @ {rand} @?")
    ...
    with tf.io.gfile.GFile("gs://e2e_central/data/probes/probe_1.tsv", 'w') as f:
      for line in probes:
        f.write(f"{line}\n")
